...
async def get_org_access_requests(org_id: str, user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)):
...
async def decide_access_request(payload: AccessDecisionPayload, user_id: str = Depends(get_current_user_id), db: AsyncSession = Depends(get_db)):
